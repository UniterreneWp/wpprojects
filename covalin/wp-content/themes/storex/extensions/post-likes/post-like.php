	...
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			<div class="favourite-posts">
				<h3><?php esc_html_e( 'You Like:', 'storex' ); ?></h3>
				<ul>
		...
		<div class="favourite-posts">
			<h3><?php esc_html_e( 'Nothing yet', 'storex' ); ?></h3>
		</div>
	<?php }
}
...
class storex_user_likes_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__('PT User Likes Widget', 'storex'), // Name
			array( 'description' => esc_html__( 'Plum Tree special widget. If the user is logged in, output a list of posts that the user likes', 'storex' ), ) 
		);
	}
...
class storex_popular_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__('Storex Popular Posts Widget', 'storex'), // Name
			array( 'description' => esc_html__( 'Plum Tree special widget. Outputs a list of the posts with the most user likes', 'storex' ), ) 
		);
	}
	...
	?>
		...
		<p>
            <label for="<?php echo $this->get_field_id("range"); ?>"><?php esc_html_e('Time Range:', 'storex'); ?></label>
        	<select class="widefat" id="<?php echo $this->get_field_id("range"); ?>" name="<?php echo $this->get_field_name("range"); ?>">
          		<option value="date" <?php selected( $instance["range"], "day" ); ?>><?php esc_html_e('Today', 'storex'); ?></option>
           		<option value="comment_count" <?php selected( $instance["range"], "week" ); ?>><?php esc_html_e('Week', 'storex'); ?></option>
         		<option value="title" <?php selected( $instance["range"], "month" ); ?>><?php esc_html_e('Month', 'storex'); ?></option>
				<option value="author" <?php selected( $instance["range"], "all" ); ?>><?php esc_html_e('All Time', 'storex'); ?></option>
        	</select>
        </p>
        <p>
	...
	public function widget( $args, $instance ) {
		...
		switch ($range) {
			case 'day':
				$today = date('j');
				$inner_title = esc_html__( 'Today\'s Most Popular Posts', 'storex' );
			break;
			case 'week':
				$week = date('W');
				$inner_title = esc_html__( 'This Month\'s Most Popular Posts', 'storex' );
			break;
			case 'month':
				$month = date('m');
				$inner_title = esc_html__( 'This Month\'s Most Popular Posts', 'storex' );
