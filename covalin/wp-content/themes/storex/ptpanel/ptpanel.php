...
class MediauploadField extends FormField{
	...
	public function setView() {
		...
		$this->_value .= '<div class="form_holder"><input class="upload_media" type="url" id="'.$this->_id.'" name="'.$this->_name.'" '.(($required) ? ' required ' :  '' ).'  value="'.(is_array(get_option($this->_name)) ? '' : get_option($this->_name) ).'" />';
					...
					jQuery(document).ready(function($){
						...
						$("#'.$this->_id.'_button").click(function(e){
							...
							wp.media.editor.send.attachment = function(props, attachment){
								...
								if (_custom_media) {
									if (attachment.type == "image") $("#"+id).val(attachment.url);
								} else {
