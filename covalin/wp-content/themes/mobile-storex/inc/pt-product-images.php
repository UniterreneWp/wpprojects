...
/*-------Storex Custom product images output----------*/

if ( class_exists('Woocommerce') ) {

	if ( ! function_exists( 'pt_product_images_output' ) ) {
		function pt_product_images_output(){

			global $post, $woocommerce, $product;

			/* Variables */
			$slider_type = (get_option('product_slider_type') != '') ? get_option('product_slider_type') : 'slider-with-thumbs';
			$transition_type = (get_option('product_slider_effect') != '') ? get_option('product_slider_effect') : 'fade';
			...
			function pt_get_main_img($size, $main_slider){
				...
				if ( has_post_thumbnail() ) {
					$image       		= get_the_post_thumbnail( $post->ID, apply_filters( 'single_product_large_thumbnail_size', $size ), $custom_attr );
			...
			if ($slider_type == 'slider-with-thumbs') {
				$html_output .= '<div class="thumb-slider"'.$extra_thumbs_owl_attr.'>';
				$html_output .= '<div class="slide">'.pt_get_main_img('shop_thumbnail', false).'</div>';
				if ( $attachment_ids ) {
					$html_output .= pt_get_attachment_imgs('shop_thumbnail', false);
				}
				$html_output .= '</div>';
			}
