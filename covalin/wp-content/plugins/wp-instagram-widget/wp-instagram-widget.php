	...
	function widget( $args, $instance ) {
		...
		if ( $username != '' ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				$liclass = apply_filters( 'wpiw_item_class', 'home-cat-colds' );
				...
				?>
                <link rel="stylesheet" href="<?php echo plugins_url( 'assets/colorbox.css', __FILE__ );?>" />                
                <script src="<?php echo plugins_url( 'assets/jquery.colorbox.js', __FILE__ );?>"></script>
		<script>
			jQuery(document).ready(function(){
				//jQuery(".iframe").colorbox({iframe:true, width:"80%", height:"80%"});
				jQuery(".iframe").colorbox({rel:'iframe', slideshow:true});
			});
		</script>
                <style>
				#wysija-3{
				...
				.instagram-pics li.home-cat-colds{
					width:17%;
					float:left;
					margin:5px;
					list-style:none;
				...
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( $template_part ) != '' ) {
						...
						echo '<li class="'. esc_attr( $liclass ) .'"><a href="'.  esc_url( $item['original'] ) .'" class="iframe '. esc_attr( $aclass ) .'" title="'. esc_attr( $item['description'] ).'"><img src="'. esc_url( $item[$size] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. esc_attr( $imgclass ) .'"/></a></li>';
					}
				}
				?></ul><?php
			}
