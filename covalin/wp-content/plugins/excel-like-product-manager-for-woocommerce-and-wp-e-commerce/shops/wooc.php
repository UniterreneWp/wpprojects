...
if(isset($_REQUEST["memtest"])){
	...
	die;
	return;
}elseif($mem_limit == 0 && !getRequestVar("remote_export") && !getRequestVar("remote_import")){
...
if(!$mem_limit && (getRequestVar("remote_export") || getRequestVar("remote_import"))){
	$mem_limit = 64000000;
}
...
global $plem_settings;
global $attribute_val_count;
global $products_rendered;
$products_rendered = false;

$attribute_val_count = NULL;
$plem_settings = $this->settings;

//var_dump($plem_settings);
...
if(isset($plem_settings['wooc_fileds'])){
	...
	$attrset_visibility  = new stdClass();
	if(!getRequestVar("do_export") && !getRequestVar("do_import")){
		
		global $attribute_settings;
		
		global $plem_settings;
		
		if(!isset($attribute_settings)){
			if(isset($plem_settings["attribute_settings"])){
				if($plem_settings["attribute_settings"]->use){
					$attribute_settings = $plem_settings["attribute_settings"];
				}else
					$attribute_settings = false;
			}else
				$attribute_settings = false;
		}
		...
		if($attribute_settings){
			if($attribute_settings->use){
				...
				if(isset($attribute_settings->hide_no_value))
					if($attribute_settings->hide_no_value)
						$attribute_val_count = array();
				
				$attv_cat   = array();
				if(isset($_REQUEST['product_category']) && isset($attribute_settings->cat)){
					$product_category = explode(",", $_REQUEST['product_category']);
					if(!empty($product_category)){
						foreach($product_category as $pc){
							if(isset($attribute_settings->cat->{$pc})){
								$attv_cat[] = $attribute_settings->cat->{$pc};
							}
						}
					}
				}
				
				foreach($attv_cat as $cata){
					foreach($cata as $prop => $value){
						if(strpos($prop,"pa_") === 0){
							if(!isset($attrset_visibility->{"pattribute_".$value->id}))
								$attrset_visibility->{"pattribute_".$value->id} = 0;
							if(isset($value->visible)){
								$attrset_visibility->{"pattribute_".$value->id} += ($value->visible ? 1 : 0);
							}
						}
					}
				}
				...
				foreach($attribute_settings as $prop => $value){
					if(strpos($prop,"pa_") === 0){
						if(isset($attribute_settings->{$prop}->visible)){
							
							if(!isset($attrset_visibility->{"pattribute_".$attribute_settings->{$prop}->id}))
								$attrset_visibility->{"pattribute_".$attribute_settings->{$prop}->id} = ($attribute_settings->{$prop}->visible ? 1 : 0) ;
							
						}
					}	
				}
				
				$all_fileds = explode(",",$plem_settings['wooc_fileds']);
				for($I = 0; $I < count($all_fileds);$I++ ){
					$val = $all_fileds[$I];
					if(strpos($val,"pattribute_") === 0){
						$wooc_fields_visible[$val] = false;
						if(isset($attrset_visibility->{$val})){
							if($attrset_visibility->{$val})
								$wooc_fields_visible[$val] = true;
						}
					}
				}
...
function fn_show_filed($name){
	global $wooc_fields_visible, $custom_export_columns, $impexp_settings,$attribute_val_count, $products_rendered;
$products_rendered = false;;
	...
	if($products_rendered){
		if(strpos($name, "pattr") === 0){
			if(isset($attribute_val_count)){
				if(isset($attribute_val_count[$name])){
					if(isset($wooc_fields_visible[$name])){
						if($wooc_fields_visible[$name]){
							if(!$attribute_val_count[$name]){
								return false;
							}
						}
					}
...
function explicitOrderProductAttributes(&$pa, $scope_pid){
	global $attribute_settings;
	$cat_ids = wp_get_post_terms($scope_pid, 'product_cat', array("fields" => "ids"));
	
	$cat_settings = null;
	if(isset($attribute_settings->cat)){
		foreach($cat_ids as $cat_id){
			if(isset($attribute_settings->cat->{$cat_id})){
				if($attribute_settings->cat->{$cat_id}->use_attr_settings_of != -1){
					if($attribute_settings->cat->{$cat_id}->use_attr_settings_of){
						$ref_cat_id = $attribute_settings->cat->{$cat_id}->use_attr_settings_of;
						if(isset($attribute_settings->cat->{$ref_cat_id})){
							$cat_settings = $attribute_settings->cat->{$ref_cat_id};
						}else
							$cat_settings = $attribute_settings->cat->{$cat_id};
					}else
						$cat_settings = $attribute_settings->cat->{$cat_id};
				}else{
					$cat_settings = $attribute_settings->cat->{$cat_id};
	...
	$pa_vals = array_values($pa);
	
	$n = 1; 
	for($i = 0; $i < count($pa_vals); $i++){
		$key = $pa_vals[$i]["name"];
		unset($pa[$key]);
		if($cat_settings){
			if(isset($cat_settings->{$key})){
				if(isset($cat_settings->{$key}->order)){
					$pa_vals[$i]["order"] = intval($cat_settings->{$key}->order);
					
				}	
			}
		}
		
		if(!isset($pa_vals[$i]["order"])){
			if(isset($attribute_settings->{$key})){
				if(isset($attribute_settings->{$key})){
					if(isset($attribute_settings->{$key}->order)){
						$pa_vals[$i]["order"] = intval($attribute_settings->{$key}->order);
	...
	usort($pa_vals, "cmpAttOrder");
	
	for($i = 0; $i < count($pa_vals); $i++){
		$key = $pa_vals[$i]["name"];
		unset($pa_vals[$i]["order"]);
		$pa[$key] = $pa_vals[$i];
		$pa[$key]["position"] = $i + 1;
	}
}

function orderProductAttributes(&$pa, $scope_pid){
	...
	if($attribute_settings){
		explicitOrderProductAttributes($pa, $scope_pid);
		return;
	}
...
if(isset($_REQUEST["REPACK_ATTRIBUTES"])){
	...
	foreach($req->p_ids as $p_id){
		$patts = get_post_meta($p_id,'_product_attributes',true);
		
		$p_a_t = $wpdb->get_col("SELECT 
							t.taxonomy
						FROM
							`$wpdb->term_taxonomy` as t 
						LEFT JOIN
							`$wpdb->term_relationships` as r on t.`term_taxonomy_id` = r.`term_taxonomy_id`
						WHERE  
							r.`object_id`  = $p_id
							AND
							t.taxonomy LIKE 'pa_%'
		...
		foreach($p_a_t as $patax){
			
			if(!isset($patts[$patax])){
				
				$patts[$patax] = array (
									'name'         => $patax,
									'value'        => '',
									'position'     => count($patts[$patax]),
									'is_visible'   => 1,
									'is_variation' => 0,
									'is_taxonomy'  => 1
								 );
								 
				update_post_meta($p_id,'_product_attributes',$patts);
				
				$false = false;   
				updateParentVariationData($p_id,$false,$attributes_asoc,array(substr($patax,3)));
				$patts = get_post_meta($p_id,'_product_attributes',true);	
			}
		}
		...
		orderProductAttributes($patts, $p_id);
		update_post_meta($p_id,'_product_attributes',$patts);
	...
	$wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE '_transient%';");
	echo '{"repack_attributes":"ok", "count":"'.$count.'"}';
	die;
}
	...
	if($defaultAtt){
		if(isset($defaultAtt[0])){
			if(is_string($defaultAtt)){
				$defaultAtt = array();
				orderProductAttributes($defaultAtt,$parent);
				if(isset($defaultAtt['']))
					unset($defaultAtt['']);
...
function clonePost($post_id, $with_parent = NULL){
	...
	if($new_post_id){
		...
		global $attributes_asoc;
		$tmp = new stdClass;
		updateParentVariationData($new_post_id,$tmp,$attributes_asoc,NULL,false);
...
if($_REQUEST['DO_UPDATE'] == '1' && strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){
	...
	foreach($tasks as $key => $task){
	   ...
	   if(property_exists($task,"variate_by")){
		   ...
		   orderProductAttributes($patts, $parent ? $parent : $key);
		   if(isset($patts['']))
			   unset($patts['']);
		...
		if ($json_import || isset($_FILES['file']['tmp_name']) || isset($_REQUEST["continueFrom"]) || isset($_REQUEST["tmp_file"]) || isset($_REQUEST["commit_import"])) {
			...
			while ($data) {
				...
				if($parent){
							...
							if(isset($data[$attribute_indexes[$attr->name]])){
								...
								if($att_value){
									...
									if($pa_dirty){
										...
										orderProductAttributes($parAttCFG, $parent);
										if($parAttCFG[""])
											unset($parAttCFG[""]);
				   ...
				   if($patts_set){
					    orderProductAttributes($patts,$id);
...
if(!isset($_REQUEST["IDS"])){
	...
	if(isset($product_name) && $product_name){
		$panmes = explode(",",$product_name);
		$panmes = array_map('trim',$panmes);
		
		$name_postids = $wpdb->get_col("select ID from $wpdb->posts where post_title rlike '". implode("|",$panmes) . "' ");
	...
	foreach($IDS as $p_id){
		...
		$par_id = wp_get_post_parent_id( $p_id ); 
		if($par_id){
			if(!in_array($par_id, $IDS)){
				$p_id = $par_id;
			}
		}
...
function product_render(&$IDS,&$attributes,$op,&$df = null){
	global $wpdb, $custom_fileds, $impexp_settings, $custom_export_columns,$resume_skip, $attribute_val_count;
	...
	foreach($p_ids as $id) {
	 ...
	 if(!$prod->parent && $ptype !== 'variable'){
	  ...
	  foreach($attributes as $att){
		...
		if(isset($attribute_val_count)){
			if(!isset($attribute_val_count['pattribute_'.$att->id])){
				$attribute_val_count['pattribute_'.$att->id] = 0;
			}
		}
		...
		if(isset($attribute_val_count)){
			if($prod->{'pattribute_'.$att->id}){
				$attribute_val_count['pattribute_'.$att->id] += 1;
			}
		}
	  }
	  ...
	  if($op == "json"){
		 ...
		 if($p_n > 0 && $start_time + $max_time  < time() || getMemAllocated() + 1048576 > $mem_limit){
		  ...
		  if($in_export_chunk == $impexp_settings->export_loop_chunk || $p_n > $resume_skip && $start_time + $max_time  < time() || getMemAllocated() + 1048576 > $mem_limit){
			$res_limit_interupted = $p_n + 1;
			break;	
		  }
 ...
 <?php
  ...
  <li>
   ...
   <ul>
	 ...
	 <li><span><button onclick="attributeSettings();return false;" ><?php echo __("Attribute settings...",'productexcellikemanager'); ?></button></span></li>
