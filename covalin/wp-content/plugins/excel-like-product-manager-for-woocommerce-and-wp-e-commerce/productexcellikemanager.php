...
if ( ! class_exists( 'productexcellikemanager' ) ) {
   ...
   class productexcellikemanager{
						...
						if(isset($this->settings[$_REQUEST["elpm_shop_com"].'_custom_import_settings'])){
							...
							if($ri_settings->allow_remote_import){
								...
								if($let_in){
									...
									$this->remoteImportError = __('Remote import/export is not allowed form origin IP address ("'.$ip.'") of request!','productexcellikemanager');
									add_action('wp_ajax_nopriv_pelm_frame_display',array( $this,'echoRemoteImportError'));
									$hooked = true;
									
								}
							}else{
								$this->remoteImportError = __('Remote import/export is not enabled in plugin settings!','productexcellikemanager');
								add_action('wp_ajax_nopriv_pelm_frame_display',array( $this,'echoRemoteImportError'));
								$hooked = true;
							}
						}else{
		...
		public function outputDebugBacktrace($arg1){
		...
		public function display($elpm_shop_com){
			...
			if($elpm_shop_com == "settings" || !isset($this->settings["APIKEY"])){
			    ...
			    if(!$this->is_internal){
					...
					$this->settings['plugin_version'] = $plugin_data['Version'];
