...
function auto_search_bar(){
    ...
    ?>
    <form role="search" method="get" id="auto-searchform" class="woocommerce-product-search" action="<?php echo esc_url( home_url( '/'  ) ); ?>">
      <ul class="group">
        <li class="search-input">
	    <input type="text" class="left textfield search-field" value="<?php echo _e( get_search_query() ); ?>" name="<?php echo esc_attr__( $name ); ?>" id="s" placeholder="<?php _e( $woo_form_placeholder, 'woocommerce' ); ?>" />
         </li>
        <li class="search-button">
        <input type="submit" id="searchsubmit" value="<?php echo esc_attr__( $woo_button_placeholder ); ?>"/>
		<?php
        ...
        <script>
            jQuery(function($){
                ...
                ';
					...
					if (get_option('autocomplete_tags') != false){
					...
					if (get_option('autocomplete_taxonomies') != false){
					...
					/*else{*/
						...
						//	print_r($val);
						$products = '{label: "'.sanitize_text_field( $val->post_title ) . '", value: "'.sanitize_text_field( $val->post_title ).'", pid: "'.$val->ID.'"},';
						//	$products = '"'.$val->ID.'^'.sanitize_text_field( $val->post_title ) . '",';
							echo $products;
						}
...
select: function (event, ui) {
	...
	var pidval=ui.item.value;
	var productid=ui.item.pid;
	document.valueSelectedForAutocomplete = label; 
	
	if( productid==null) {
		$( "#sid" ).val(productid);
	  $( "#s" ).attr("name","product_cat");
		}else{
	$( "#sid" ).val(productid);
	  $( "#s" ).attr("name","s");
	}
...
function woo_autocomplete_options_page() {
    ?>
		<style>
			.form-table td{padding: 10px !important; color: black;}
			.form-table th{padding: 10px !important; color: black;}
		    .woo_auto_admin_spans{font-size: 10px; color: red;}
		</style>
		<div class="wrap">
...
function exclude_taxonomy_element(){
	?>
	 ...
	 <script>
	 $('input.wasbcheckbox').on('change', function() {   
		 $('input.wasbcheckbox').not(this).prop('checked', false);  
	 });
	 </script>
	<?php
}

/*------ Add Jquery to admin panel for selectable checkboxes ---------*/
/*function my_enqueue() {
    global $pagenow;
	 // Check for admin and admin page to avoide jQuery Conflict
    if ( is_admin() && $pagenow == 'admin.php' ){
		  wp_enqueue_script( 'wasb_jquery', "http://code.jquery.com/jquery-1.7.2.min.js" );	 
	}
}
add_action( 'admin_esnqueue_scripts', 'my_enqueue' );*/
...
add_action('template_redirect', 'search_productpage_redirect');
function search_productpage_redirect() {
    if (is_search()) {
        global $wp_query;
		 $serchkeyword=$wp_query->query_vars['s'];	
		 $pid=$wp_query->query_vars['sid'];	
		 
		 if($pid!=''){			
wp_redirect(get_permalink($pid));
			}elseif (!empty($serchkeyword)||($wp_query->query_vars['post_type'] == 'product')) {
			//$serchkeyword='C2201TR 15A, 125Vac, Tamper Resistant Duplex Receptacle';	

			$product = get_page_by_title($serchkeyword,$output = OBJECT, $post_type = 'product');	
		if($product->ID==''){
				return $wp_query;
				}else{	
