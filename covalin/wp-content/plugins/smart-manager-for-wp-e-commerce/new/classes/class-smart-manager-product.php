...
if ( ! class_exists( 'Smart_Manager_Product' ) ) {
	class Smart_Manager_Product extends Smart_Manager_Base {
		...
		public function search_terms_condition_complete($result_terms_search = array(), $search_params = array()) {
            ...
            if (!empty($result_terms_search) && !empty($search_params) && (trim($search_params['cond_terms_col_name']) == 'product_cat' || trim($search_params['cond_terms_col_name']) == 'product_type' || trim($search_params['cond_terms_col_name']) == 'product_visibility') ) {
                ...
                if ( $this->terms_att_search_flag == 1 ){
                    $query = "REPLACE INTO {$wpdb->base_prefix}sm_advanced_search_temp
                            ( SELECT {$wpdb->prefix}posts.id, ". $this->terms_att_search_flag ." ,1
		...
		public function sm_search_query_terms_select($sm_search_query_terms_select = '', $search_params = array()) {
			...
			if ( !empty($search_params['cond_terms_col_name']) && trim($search_params['cond_terms_col_name']) != 'product_cat' && trim($search_params['cond_terms_col_name']) != 'product_type' && trim($search_params['cond_terms_col_name']) != 'product_visibility' ) {
		        $sm_search_query_terms_select .= " ,0";
		        $this->terms_att_search_flag = 1; //Flag to handle the child ids for cat advanced search
		    } else if ( !empty($search_params['cond_terms_col_name']) && (trim($search_params['cond_terms_col_name']) == 'product_cat' || trim($search_params['cond_terms_col_name']) == 'product_type'  || trim($search_params['cond_terms_col_name']) == 'product_visibility' ) ) {
		...
		public function sm_search_query_terms_where($sm_search_query_terms_where = '', $search_params = array()) {
			...
			if ( !empty($search_params['cond_terms_col_name']) && trim($search_params['cond_terms_col_name']) != 'product_cat' && trim($search_params['cond_terms_col_name']) != 'product_type' && trim($search_params['cond_terms_col_name']) != 'product_visibility' ) {
				...
				$taxonomy_cond = (!empty($taxonomy_cond)) ? ' ( '. $taxonomy_cond .' OR ' : '';	

		        $sm_search_query_terms_where = " WHERE ". $taxonomy_cond ."
                                                        OR ({$wpdb->prefix}postmeta.meta_key ". (($search_params['cond_terms_col_value'] == "''") ? 'LIKE' : $search_params['cond_terms_operator']) ." 'attribute_".trim($search_params['cond_terms_col_name']) . 
		...
		public function sm_search_query_postmeta_where($sm_search_query_postmeta_where = '', $search_params = array()) {
			...
			if(!empty( $search_params ) && !empty( $search_params['cond_postmeta_col_name'] ) ) {
				if( $search_params['cond_postmeta_col_name'] == '_regular_price' || $search_params['cond_postmeta_col_name'] == '_sale_price' ) {
	               $sm_search_query_postmeta_where .= " AND wp_postmeta.post_id NOT IN (SELECT post_parent 
	                                                                  FROM {$wpdb->prefix}posts
	            ...
	            if( $search_params['cond_postmeta_col_name'] == '_product_attributes' ) {
	            	$index = strpos($postmeta_advanced_search_where, 'WHERE');
		            if( $index !== false ){
		            	$sm_search_query_postmeta_where = substr($postmeta_advanced_search_where, $index);
		            }
		        	
		        	$sm_search_query_postmeta_where = " WHERE ( (". $sm_search_query_postmeta_where .") OR (wp_postmeta.meta_key LIKE 'attribute%' AND wp_postmeta.meta_value ". $search_params['cond_postmeta_operator'] ." '%". $search_params['cond_postmeta_col_value'] ."%') ) ";
	            }
			}
